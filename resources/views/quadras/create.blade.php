<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-gray-600 text-sm">
            ...
            <span>/</span>
            <span class="text-gray-900 font-semibold">Nova Quadra</span>
        </div>
    </x-slot>

    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="{{ route('condominios.quadras.store', $condominio) }}" method="POST" class="space-y-6">
                @csrf
                <div class="bg-white shadow-sm rounded-lg p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="area_total" value="{{ old('area_total') }}"
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Observações</label>
                        <textarea name="observacoes" rows="3"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('observacoes') }}</textarea>
                    </div>
                </div>
